...
1. Send command to the subsystem directly by setting the to_ip (local ip of subsystem) and to_port (local port of subsystem) in api.API(). Data is not 
   stored in the SQLight database but faster.
2. Send command to the database by setting to_ip (host.docker.internal) and to_port (65531) in api.API(), and make sure the dataID is 
