...
func main() {
	...
	app.GET("/ws", func(ctx *sgo.Context) error {
	...
	app.Run(":10123")
	// fmt.Println(call(PyFuncParams{NumTimeSteps: 2, NumSamples: 2}))
}
...
func call(params PyFuncParams) (PyFuncOutput, error) {
	...
	outputJSON, err := exec.Command("python3", "-u", "../cdcm_hab/examples/HCI_CDCM_DT/thermal_dt_script.py", string(inputJSON)).Output()
	if err != nil {
		return output, err
	}
